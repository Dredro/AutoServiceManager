        ...
        public static void GenerateInvoice(string filePath, OrderDTO order)
        {
            ...
            var document = QuestPDF.Fluent.Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.Header().Text("FAKTURA VAT nr"+order.Id)
                        .SemiBold().FontSize(20).FontColor(Colors.Blue.Medium);

                    page.Content().AlignLeft().Text($"Data ukończnia usługi: {order.FinalizationDate}")
                        .FontSize(12);

                    page.Content().AlignLeft().Text($"Klient: {order.Client?.PersonalInfo?.FirstName} {order.Client?.PersonalInfo?.LastName}").FontSize(12);
                    if(order.Vehicle != null)
                    {
                        page.Content().AlignLeft().Text($"Pojazd: {order.Vehicle.Make} {order.Vehicle.Model} ({order.Vehicle.RegistrationNumber})").FontSize(12);
                    }
                    page.Content().AlignCenter().Text($"Wykonane usługi").FontSize(16).FontColor(Colors.Blue.Medium);
                    ...
                    page.Content().AlignCenter().Text($"Łączny koszt usług: {order.ServicesToDo.Sum(s => s.Price):F2} zł").FontSize(16).FontColor(Colors.Green.Darken1);

                    page.Content().AlignCenter().Text($"Wykorzystane części zamienne").FontSize(16).FontColor(Colors.Blue.Medium);
                    ...
                    page.Content().AlignCenter().Text($"Łączny koszt części zamiennych: {order.SpareParts.Sum(p => p.SparePart?.Price * p.Quantity):F2} zł").FontSize(16).FontColor(Colors.Green.Darken1);
                    page.Content().AlignCenter().Text($"Łączny koszt zamówienia: {order.TotalCost:F2} zł").FontSize(18).FontColor(Colors.Green.Darken1);
